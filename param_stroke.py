...
def stroke_renderer(curve_points: T.Tensor, locations: T.Tensor, colors: T.Tensor, widths: T.Tensor,
    ...
    I_colors = T.einsum('hwnf,hwn->hwf', canvas_with_nearest_Bs_colors.float(), I_NNs_B_ranking.float())  # [H, W, 3]
    bs = T.einsum('hwnf,hwn->hwf', canvas_with_nearest_Bs_bs.float(), I_NNs_B_ranking.float())  # [H, W, 1]
...
class BrushStrokeRenderer(T.nn.Module):
    def __init__(self, canvas_height, canvas_width, num_strokes=5000, samples_per_curve=10, strokes_per_pixel=20,
                 canvas_color='gray', length_scale=1.1, width_scale=.1, content_img=None, style_image=None, init='random'):
        super().__init__()
        ...
        if content_img is not None:
            location, s, e, c, width, color = utils.initialize_brushstrokes(content_img, num_strokes,
                                                                            ...
                                                                            length_scale, width_scale, init=init, style_image=style_image)
        else:
            location, s, e, c, width, color = utils.initialize_brushstrokes(content_img, num_strokes,
                                                                            canvas_height, canvas_width,
