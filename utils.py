...
def create_center_grid(a_min, a_max, canvas_height, canvas_width, canvas):
    step_vertical = int(a_min // 6)
    step_horizontal = int(a_max // 6) 
    
    print(canvas_width, canvas_height)
    print(canvas.shape)
    
    x_coords = np.arange(0, canvas_width, step_horizontal)
    y_coords = np.arange(0, canvas_height, step_vertical)
    ...
    for x in x_coords:
        for y in y_coords:
            location.append([y + step_vertical  // 2,
                            x + step_horizontal // 2])
            s.append([np.random.uniform(low=-1, high=1),
                      -1. * step_horizontal // 2 + np.random.uniform(low=-1, high=1)])
            e.append([np.random.uniform(low=-1, high=1),
                      step_horizontal // 2 + np.random.uniform(low=-1, high=1)])
            c.append([np.random.uniform(low=-1, high=1),
                      np.random.uniform(low=-1, high=1)])
            width .append(step_vertical )
            print(s[-1])
            print(canvas[y,
                                x,
                                :])
            color.append(canvas[y,
                                x,
                                :]) 
                             
            
    location, s, e, c, width, color = [np.array(x).astype(np.float32) for x in [location, s, e, c, width, color]]
    return location, s, e, c, width, color
     



def initialize_brushstrokes(content_img, num_strokes, canvas_height, canvas_width, sec_scale, width_scale, init='sp', style_image=None):
    if init == 'random':
        # Brushstroke colors
        ...
        a_min, a_max = get_contours.get_ellipse_stats(style_image, n_iters = 1000, need_graphics = False)
        
        location, s, e, c, width, color = create_center_grid(a_min, a_max, canvas_height, canvas_width, content_img) 
        ...
        plt.imshow(mark_boundaries(content_img, segments))
        
        plt.savefig("mygraph_2023.png")
